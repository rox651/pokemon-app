...
async function getTypeEffectiveness(
   ...
   for (const defenderType of defenderTypes) {
      if (damage_relations.no_damage_to.some(type => type.name === defenderType)) {
         return 0;
      }

      if (damage_relations.double_damage_to.some(type => type.name === defenderType)) {
         effectiveness *= 2;
      } else if (damage_relations.half_damage_to.some(type => type.name === defenderType)) {
         effectiveness *= 0.5;
      }
   }
...
export default async function calculateDamage(
   ...
   const normalAttack = attacker.stats.find(stat => stat.stat.name === "attack")?.base_stat ?? 0;
   const specialAttack =
      attacker.stats.find(stat => stat.stat.name === "special-attack")?.base_stat ?? 0;

   const defenderDefense =
      defender.stats.find(stat => stat.stat.name === "defense")?.base_stat ?? 0;
   const defenderSpecialDefense =
      defender.stats.find(stat => stat.stat.name === "special-defense")?.base_stat ?? 0;

   const move = await repository.fetchPokemonMoveByName(pokemonMove.move.name);

   const attackStat = move.damage_class.name === "physical" ? normalAttack : specialAttack;

   const defenseStat =
      move.damage_class.name === "physical" ? defenderDefense : defenderSpecialDefense;
